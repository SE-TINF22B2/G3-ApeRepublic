...
public class UserAuthService {
    ...
    public ResponseEntity<String> processRegisterUser(UserRegisterRequest userRegisterRequest) {
        ...
        if(!userRegisterRequest.isValid()) {
            return ResponseEntity.ok(new APIResponse(ResponseStatus.MISSING_PARAMS).build());
        }
        if(mockUserRepository.existsByUsername(userRegisterRequest.username)) {
            return ResponseEntity.ok(new APIResponse(ResponseStatus.USERNAME_TAKEN).build());
        }
        if(mockUserRepository.existsByEmail(userRegisterRequest.email)) {
            return ResponseEntity.ok(new APIResponse(ResponseStatus.EMAIL_TAKEN).build());
        }
        ...
        return ResponseEntity.ok(new APIResponse(ResponseStatus.SUCCESSFUL_REGISTER).addSessionTokenId(sessionToken.toString()).addUserDetails(userDetails).build());
    }
    ...
    public ResponseEntity<String> processLoginUser(UserLoginRequest userLoginRequest) {
        ...
        if(optRequestUser.isEmpty()) {
            return ResponseEntity.ok(new APIResponse(ResponseStatus.UNSUCCESSFUL_LOGIN).build());
        }
        ...
        return ResponseEntity.ok(new APIResponse(ResponseStatus.SUCCESSFUL_LOGIN).addSessionTokenId(sessionTokenId).addUserDetails(UserDetails.build(requestUser)).build());
    }
    ...
    public ResponseEntity<String> processLogoutUser(UserLogoutRequest userLogoutRequest) {
        if(!activeUserService.removeIfTokenMatch(userLogoutRequest.token)) {
            return ResponseEntity.ok(new APIResponse(ResponseStatus.UNSUCCESSFUL_LOGOUT).build());
        }
        return ResponseEntity.ok(new APIResponse(ResponseStatus.SUCCESSFUL_LOGOUT).build());
    }

    public ResponseEntity<String> processDeleteUser(UserLoginRequest userLoginRequest) {
        return ResponseEntity.ok(new APIResponse(ResponseStatus.ERROR).build());
    }
